...
export const buyerAuthenticate = async (
	req: Request,
	res: Response,
	next: NextFunction
): Promise<void> => {
	try {
        ...
        console.log(user);
		if (user && user?._id && user?.userType === 'BUYER') {
			req.body.user = user;
		} else if(user!.userType !== 'BUYER' ){
            res.status(StatusCodes.UNAUTHORIZED).json({ message: MIDDLEWARE.FAILURE.NOT_AUTHORIZED });
            return;
        }else {
